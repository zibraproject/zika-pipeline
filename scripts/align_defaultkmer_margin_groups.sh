...
tmp=`tempfile`
poretools fastq --type 2D --group $group ../data/$poretools_dir/pass > $tmp
poretools fasta --type 2D --group $group ../data/$poretools_dir/pass > "$tmp".fasta
if [ "$read_type" == "hq" ];
then
	poretools fastq --type 2D --group $group --high-quality ../data/$poretools_dir/fail >> $tmp
	poretools fasta --type 2D --group $group --high-quality ../data/$poretools_dir/fail >> "$tmp".fasta
elif [ "$read_type" == "pass" ]
then
	echo "nothing"
fi
...
if [ ! "$second_batch" == "na" ]
then
	poretools fastq --type 2D --group $group ../data/$second_batch/pass >> $tmp
	poretools fasta --type 2D --group $group ../data/$second_batch/pass >> "$tmp".fasta
	if [ "$read_type" == "hq" ];
	then
        	poretools fastq --type 2D --group $group --high-quality ../data/$second_batch/fail >> $tmp
        	poretools fasta --type 2D --group $group --high-quality ../data/$second_batch/fail >> "$tmp".fasta
	elif [ "$read_type" == "pass" ]
	then
        	echo "nothing"
	fi
...
mungeheaders.py $tmp "fastq" > "$tmp".munged
mv "$tmp".munged "$sample_tag".fastq
mungeheaders.py "$tmp".fasta "fasta" > "$tmp".fasta.munged
mv "$tmp".fasta.munged "$sample_tag".fasta
...
nanopolish variants -vvvv --models-fofn offset_models.fofn --progress -t 1 --reads "$sample_tag".fasta -o np_"$ref_prefix"_"$sample_tag".vcf -b "$ref_prefix"_"$sample_tag"_marginalign.sorted.bam -e "$ref_prefix"_"$sample_tag"_np.sorted.bam -g ../refs/"$ref_prefix".fasta -vv -w "EM_079517:0-20000" --snp
