...
tmp=`tempfile`
poretools fastq --type fwd ../data/$poretools_dir/pass > $tmp
poretools fasta --type fwd ../data/$poretools_dir/pass > "$tmp".fasta
if [ "$read_type" == "hq" ];
then
	poretools fastq --type fwd --high-quality ../data/$poretools_dir/fail >> $tmp
	poretools fasta --type fwd --high-quality ../data/$poretools_dir/fail >> "$tmp".fasta
elif [ "$read_type" == "pass" ]
then
	echo "nothing"
fi
...
if [ ! "$second_batch" == "na" ]
then
	poretools fastq --type fwd ../data/$second_batch/pass >> $tmp
	poretools fasta --type fwd ../data/$second_batch/pass >> "$tmp".fasta
	if [ "$read_type" == "hq" ];
	then
        	poretools fastq --type fwd --high-quality ../data/$second_batch/fail >> $tmp
        	poretools fasta --type fwd --high-quality ../data/$second_batch/fail >> "$tmp".fasta
	elif [ "$read_type" == "pass" ]
	then
        	echo "nothing"
	fi
