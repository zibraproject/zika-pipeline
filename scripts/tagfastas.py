...
for sample in runsamples['data']:
	if not os.path.exists('%s/%s.vcf' % (sample['Run::run_name'], sample['sample_id'])):
		print "No vcf for %s" % ( sample['sample_id'], )
		continue
	...
	cmd = "margin_cons.py refs/Zika_FP.fasta %s/%s.vcf %s/%s.primertrimmed.sorted.bam" % (sample['Run::run_name'], sample['sample_id'], sample['Run::run_name'], sample['sample_id'])
...
"""
	...
	rec.id = "%s|%s|%s|%s|%s" % (metadata['sample_id'], sample['Run::run_name'], metadata['municipality'], metadata['state'], metadata['collection_date'])
