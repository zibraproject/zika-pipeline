...
sample_tag="$sample_tag""$duration_secs"s

tmp=`tempfile`
poretools fastq --type 2D --start $start_time --end $end_time ../data/$poretools_dir/pass > $tmp
poretools fasta --type 2D --start $start_time --end $end_time ../data/$poretools_dir/pass > "$tmp".fasta
if [ "$read_type" == "hq" ];
then
	poretools fastq --type 2D --start $start_time --end $end_time --high-quality ../data/$poretools_dir/fail >> $tmp
	poretools fasta --type 2D --start $start_time --end $end_time --high-quality ../data/$poretools_dir/fail >> "$tmp".fasta
elif [ "$read_type" == "pass" ]
then
	echo "nothing"
fi
...
if [ ! "$second_batch" == "na" ]
then
	poretools fastq --type 2D --start $start_time --end $end_time ../data/$second_batch/pass >> $tmp
	poretools fasta --type 2D --start $start_time --end $end_time ../data/$second_batch/pass >> "$tmp".fasta
	if [ "$read_type" == "hq" ];
	then
        	poretools fastq --type 2D --start $start_time --end $end_time --high-quality ../data/$second_batch/fail >> $tmp
        	poretools fasta --type 2D --start $start_time --end $end_time --high-quality ../data/$second_batch/fail >> "$tmp".fasta
	elif [ "$read_type" == "pass" ]
	then
        	echo "nothing"
	fi
